...
public class MemberService {
	...
	public int updateNameAndRole(int id, String name, Role role) {
		return template.update("update members set name = :name, role = :role where id = :id",
				Map.of("id", id, "name", name, "role", role.name()));
	}

	public int changePassword(String loginId, String oldPass, String newPass) {

		var secret = template.queryForObject("select password from members where login_id = :loginId",
		...
		return template.update("update members set password = :password where login_id = :loginId",
				Map.of("loginId", loginId, "password", newPass));
	}

	public int changeState(String loginId, boolean active) {
		return template.update("update members set active = :active where login_id = :loginId",
				Map.of("loginId", loginId, "active", active));
	}

	public int delete(int id) {
		return template.update("delete from members where id = :id", Map.of("id", id));
	}
	
	public List<MemberVO> search(Role role, String name, Boolean status) {
		
		var sb = new StringBuffer("select * from members where 1 = 1");
